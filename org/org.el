...
(setq possible-time-in-day (list "00:00" "00:15" "00:30" "00:45"
...
(defconst org-speed-commands-default
  '(
    ...
    ("h" . (lambda ()
             (interactive)
             (if (org-at-table-p)
                 (org-export-table-to-html)
               (org-clockhistory-insert))
             ))
    ("s" . (lambda ()
             (interactive)
             (if (org-at-table-p)
                 (org-export-table-to-sql)
               (org-clockhistory-sum))
             ))
    ("Meta Data Editing")
    ("t" . org-todo)
    ("0" . (org-priority ?\ ))
