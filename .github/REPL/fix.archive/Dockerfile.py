FROM mcr.microsoft.com/azureml/openmpi4.1.0-ubuntu20.04:20231011.v1 as lasertag_install
...
COPY 0014-support-create_quiz-and-create_insight-schema.patch src/lasertag
RUN cd src/lasertag \
    ...
    && git am 0014-support-create_quiz-and-create_insight-schema.patch \
    && cd /opt/miniconda/lib/python3.8/site-packages

COPY requirements.txt /opt/
RUN pip install -r /opt/requirements.txt

COPY 0001-MI-auth-for-Azure-OpenAI.patch jupyter_ai_magics
COPY 0002-fix-raise-on-complete-multienv-auth_strategy.patch jupyter_ai_magics
COPY 0003-fix-pydantic-field-set.patch jupyter_ai_magics
COPY 0004-escape-GPT-json-output.patch jupyter_ai_magics
COPY 0005-O1-default-temperature-have-to-be-1.0.patch jupyter_ai_magics
RUN cd /opt/miniconda/lib/python3.8/site-packages/jupyter_ai_magics \
    && git apply 0001-MI-auth-for-Azure-OpenAI.patch \
    && git apply 0002-fix-raise-on-complete-multienv-auth_strategy.patch \
    && git apply 0003-fix-pydantic-field-set.patch \
    && git apply 0004-escape-GPT-json-output.patch \
    && git apply 0005-O1-default-temperature-have-to-be-1.0.patch

COPY 0001-Azure-AD-provider-for-OpenAI-auth.patch langchain_openai
RUN cd /opt/miniconda/lib/python3.8/site-packages/langchain_openai \
    && git apply 0001-Azure-AD-provider-for-OpenAI-auth.patch
