...
var outputPrettyErrors = function (filename, errors) {
    ...
    errors.forEach(function (e, i) {
        if (e) {
            ...
            e.message = e.message || '';
            results.push(pad, e.code || '', '\n    ', e.message.replace(/^\s*/, ''), ' // Line ', e.line + 1, ', Pos ', e.column, '\n');
        }
    });
    return results.join('');
};
...
var outputTerseErrors = function (filename, errors) {
    ...
    for (i = 0; i < errors.length; i += 1) {
        e = errors[i];
        if (e) {
            output.push('Lint at line ', e.line, ' character ', e.column, ': ', e.message, '\n');
            output.push((e.code || '').replace(/^\s*(\S*(\s+\S+)*)\s*$/, "$1"), '\n');
            output.push('\n');
        }
    }
    return filename + '\n' + output.join('');
...
app.post('/jslint', function (req, res) {
    ...
    function doLint(sourcedata) {
        var jslint_result, results, warn_error;
        warn_error = [];
        jslint_result = jslint(sourcedata, jslint_options);
        try {
            warn_error = warn_error.concat(jslint_result.warnings);
        } catch (e) {
            console.log(e);
        }
        if (warn_error.length === 0) {
            results = "jslint: No problems found in " + req.body.filename + "\n";
        } else {
            console.log(`find ${warn_error.length} errors`);
            if (req.body.pretty) {
                results = outputPrettyErrors(req.body.filename, warn_error);
            } else {
                results = outputTerseErrors(req.body.filename, warn_error);
            }
        }
        return results;
    }
