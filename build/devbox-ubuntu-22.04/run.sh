caffeinate docker buildx build  --no-cache --progress=plain --platform linux/arm64 --build-arg https_proxy=socks5://host.docker.internal:8080 . -t caapi/arm64.devbox.build:22.04
...
docker stop -t 0 devbox
docker rm devbox
docker run --name devbox -it  -v /Users/bartuer:/opt -w /opt/Downloads ubuntu:22.04 /bin/bash
