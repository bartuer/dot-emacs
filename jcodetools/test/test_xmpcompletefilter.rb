...
class TestXMPCompleteFilter < Test::Unit::TestCase
  ...
  def test_prepare_line
    ...
    assert_equal "var test_obj = {\n  a: [1,2,3] ,\n  d: {one:1,\n  two: '2'\n  },\n  b:false,\n  p: defun,\n  place: cook\n};\n(function(jct_target) {  var members = [];  var jct_k;  for (jct_k in jct_target) {    var o;    try {      o = jct_target[jct_k];    } catch(e) {      o = null;      continue;    }    if (o === null) {      members.push('!__XMP_MARKER__![1] ~=> ' + jct_k + ':null' + '|' + jct_k);    } else if (o.constructor === Array) {      members.push('!__XMP_MARKER__![1] ~=> ' + jct_k + '[' + o.length + ']' + '|' + jct_k + '[' + (o.length - 1) + ']');    } else if (typeof o === 'function') {      var a = o.toString().split('\\n');      var sig = jct_k + a[0].replace(/function *[a-zA-Z0-9_]*(\\(.*\\)) *\\{/g, '$1');      members.push('!__XMP_MARKER__![1] ~=> ' + sig + '|' + sig);    } else if (/\\[object .*\\]/.test(o.toString())) {      var type = o.toString().match(/\\[object (.*)\\]/)[1];       members.push('!__XMP_MARKER__![1] ~=> ' + jct_k + '{ ' + type + ' }' + '|' + jct_k + '{}');    } else if (typeof o === 'string'){      members.push('!__XMP_MARKER__![1] ~=> ' + jct_k + ' str: ' + o +  '|' + jct_k);    } else if (typeof o === 'number'){      members.push('!__XMP_MARKER__![1] ~=> ' + jct_k + ' num: ' +  o +  '|' + jct_k);    } else if (typeof o === 'boolean'){      members.push('!__XMP_MARKER__![1] ~=> ' + jct_k + ' boolean: ' + o +  '|' + jct_k);    } else {      members.push('!__XMP_MARKER__![1] ~=> ' + jct_k + '|' + jct_k);    }  }  repl.print(members.join('\\n'));})(test_obj);function defun(name, len, from, to) {\n\n}\n\n\nfunction cook(chicken,pork,beef) {\n\n}\n\n", before_exec_code.gsub(/XMP[0-9]*_[0-9]*_[0-9]*/,'__XMP_MARKER__')
  end
